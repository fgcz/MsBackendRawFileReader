    ...
    namespace MsBackendRawFileReader
    {
		...
		// https://github.com/cpanse/MsBackendRawFileReader/issues/12
            ...
            public double[] GetScanFrequency(string filterString = "", int n = 100)
            {
                ...
                for (int scanNumber = filteredScanNumbers[0];
                    scanNumber < filteredScanNumbers[filteredScanNumbers.Length - 1];
                    scanNumber += stepsize)
                {
                    var scanStatistics = rawFile.GetScanStatsForScanNumber(scanNumber);
            ...
            public double[] GetSpectrumCharges(int scanNumber)
            {
                var scanStatistics = rawFile.GetScanStatsForScanNumber(scanNumber);
                if (scanStatistics.IsCentroidScan)
                {
            ...
            public double[] GetSpectrumBaselines(int scanNumber)
            {
                var scanStatistics = rawFile.GetScanStatsForScanNumber(scanNumber);
                if (scanStatistics.IsCentroidScan)
                {
            ...
            public double[] GetSpectrumResolutions(int scanNumber)
            {
                var scanStatistics = rawFile.GetScanStatsForScanNumber(scanNumber);
                if (scanStatistics.IsCentroidScan)
                {
            ...
            public double[] GetSpectrumMasses(int scanNumber)
            {
