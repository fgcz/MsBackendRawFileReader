...
```{r filterIons, fig.retina=3, fig.height=12}
beRaw |> 
...
b <- lapply(c(1000,10000), function(n){
  
  
  lapply(c(8,16,32,64,128,256,8,16,32,64,128,256,8,16,32,64,128,256), function(size){
    start_time <- Sys.time()
    rv <- MsBackendRawFileReader:::.RawFileReader_read_peaks(rawfileEH4547, 1:n, maxGroupSize = size)
    end_time <- Sys.time()
    d <- end_time - start_time
    message(sprintf("%d - %s", size, d))
    data.frame(size=size, time=d/n, n=n)
  })
})
...
```{r}
lattice::bwplot(time ~ size | n, data=do.call('rbind', unlist(b, recursive = FALSE)), allow.multiple=T, horizontal=F)
```
