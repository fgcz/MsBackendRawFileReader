...
The code snippet above applies the defined `filterIons` method and returns `r length(X)` spectra.
The output is shown below.

Next, we define and apply a method for graphing `LGGNEQVTR` peptide spectrum matches.
Also, the function returns some statistics of the match.
...
```{r combinePeaks, fig.retina=3, fig.cap = "Combined peptide spectrum plot."}
X |>
...
The figure below displays the time performance in dependency from the chunk size
(how many spectra are read in one function call) for different.
...
TODO(cp): We compare the output of the Termo Fischer Scientif raw files versus
their corresponding mzXML files using
`Spectra::MsBackendMzR` relying on the
