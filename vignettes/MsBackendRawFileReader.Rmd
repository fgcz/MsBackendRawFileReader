...
a Mascot Generic Format [mgf](http://www.matrixscience.com/help/data_file_help.html#GEN) file which is compatible for conducting an MS/MS Ions Search using Mascot Server (>=2.7) @Perkins1999.
...
Next, we generate a mgf file for each scan type. This is helpful, e.g, for optimizing search settings tandem mass spectrometry sequence database search tool as comet @comet2012 or mascot server @Perkins1999.
...
top_n <- function(x, n = 10, mass_window = 100, ...){
  ...
  idx <- lapply(seq(0, max(x[, 1]), by = mass_window), function(mZ){
...
```{r plotSpectraMirror9594, fig.retina=3, fig.cap = "Spectra mirror plot of the filtered  (bottom) and unfiltered scan 9594."}
